...
class TestStatusMessage:
    ...
    def to_json(self):
        return json.dumps({'TestId': self.test_id, 'Status': self.status})
...
class DiagnosisTestResult:
    ...
    """
    def __init__(self, timestamp, situation_name, step_name, constraints, eval_result):
    ...
    def from_json(cls, json_in):
        json_parsed = json.loads(json_in)
        return DiagnosisTestResult(timestamp=json_parsed['Timestamp'],
                                   situation_name=json_parsed['SituationName'], step_name=json_parsed['StepName'],
                                   constraints=json_parsed['CondName'], eval_result=json_parsed['EvalResult'])

    def as_dict(self):
        return {'Timestamp': self.timestamp,'SituationName': self.situation_name,
                'StepName': self.step_name, 'CondName': self.constraints, 'EvalResult': self.eval_result}
...
class UlisesTestHelper:
    ...
    def __init__(self, mqtt_host='10.10.0.1', mqtt_port=1883, mqtt_user='', mqtt_password=None,
        ...
        self.test_info_topic = 'ULISES_test'
    ...
    def __test_result_message_callback__(self, client, userdata, message):
        ...
        :param message: MQTT message containing a topic and a payload
        """
        rospy.logwarn('Diagnosis topic received:' + message.payload)
        try:
            if self.test_result is None:
                self.test_result = DiagnosisTestResult.from_json(message.payload)
            if self.test_result:
                if self.test_result.eval_result:
                    self.test_result = DiagnosisTestResult.from_json(message.payload)
                rospy.loginfo('Test result received from tool:' + self.test_result.constraints)
        except Exception as e:
            rospy.logerr('Unable to parse diagnosis test result, exception: %s' % e)
    ...
    def wait_assert_passed(self, timeout=30):
        ...
        """
        start_time = datetime.now()
        rospy.logwarn("Result:"+ self.test_result.constraints)
